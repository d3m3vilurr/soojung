...
{if $entry->isSetOption("NO_TRACKBACK") == false}| <a class="plink" href="#none" onclick="return fold_sidebar('div_TB_{$entry->entryId}');">TrackBacks ({$entry->getTrackbackCount()})</a>{/if}
					...
					<div id="div_TB_{$entry->entryId}" style="text-align:left;">
						<div class="graybox">
							<div class="gray8" style="text-align:right; padding: 5px;">
								&nbsp;Track this back : <span style="font-size:7pt;" onclick="copy_clip(this.innerHTML);" onMouseOver="this.style.cursor='pointer';">{$baseurl}/trackback.php?blogid={$entry->entryId}</span>
							</div>
							{foreach from=$trackbacks item=trackback}
							<div class="blue8" style="width: 100%; margin: 5px 5px 0 0; color: #999999;">
								<img src="{$baseurl}/templates/{$skin}/imgs/comment.gif" alt="" /> Tracked from <a href="{$trackback->url|escape}" onclick="seturltarget(this,'_blank');"><b>{$trackback->name|escape}</b></a>
								at {$trackback->date|date_format:"%y/%m/%d %H:%M"}
							</div>
							<div class="blue8" style="margin: 0 0 5px 15px;  color: #555555;">
								<a href="{$trackback->url|escape}" onclick="seturltarget(this,'_blank');">{$trackback->title|escape}</a><br />
					...
					<div id="div_CO_{$entry->entryId}" style="text-align: left;">
						<div class="graybox">
							...
							<div class="blue8" style="margin: 5px 5px 0 0; color: #999999;">
								<img src="{$baseurl}/templates/{$skin}/imgs/comment.gif" alt="" /> Commented by<b>
									{if $comment->homepage != ""}
									<a href="{$comment->homepage|escape}" onclick="seturltarget(this,'_blank');">{$comment->name|escape}</a>
